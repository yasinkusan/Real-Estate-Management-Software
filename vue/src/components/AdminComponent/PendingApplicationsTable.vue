        ...
        <div class="panel">
          <div class="panel-heading">
            <div class="row">
              <div class="col col-sm-3 col-xs-12">
                <h4 class="title">Pending <span>Applications</span></h4>
              </div>
          ...
          <div class="panel-body table-responsive">
            <table class="table">
              ...
              <tbody>
                <tr
                  v-for="application in filteredAppllications"
                  :key="application.applicationId"
                >
                  <td>{{ application.applicationDate }}</td>
                  <!-- <td>
                    {{ application.address.addressLine }},
                  ...
                  <td>
                    <ul class="action-list">
                      <li>
                        <b-button
                          v-on:click="approveStatus(application.applicationId)"
                          variant="success"
                          >Approve</b-button
                        >
                      </li>
                      <li>
                        <b-button
                          v-on:click="denyStatus(application.applicationId)"
                          variant="danger"
                          >Deny</b-button
                        >
                      </li>
...
export default {
  ...
  created() {
    propertyService
      .retrieveApplicationsByOwnerId(this.$store.state.user.id)
      .then((response) => {
  ...
  methods: {
    approveStatus(applicationId) {
      this.application.status = "approved";
      propertyService.updateApplicationStatus(applicationId, this.application);
    },
    denyStatus(applicationId) {
      this.application.status = "declined";
      propertyService.updateApplicationStatus(applicationId, this.application);
    },
