        ...
        <div class="panel">
          <div class="panel-heading">
            <div class="row">
              <div class="col col-sm-3 col-xs-12">
                <h4 class="title">Pending Maintenance Requests</h4>
              </div>
          ...
          <div class="panel-body table-responsive">
            <table class="table">
              ...
              <tbody>
                <tr
                  v-for="maintenance in maintenanceRequests"
                  :key="maintenance.maintenanceId"
                >
                  <td>{{ maintenance.date }}</td>
                  <td>{{ maintenance.firstName }}</td>
                  <td>{{ maintenance.lastName }}</td>
                  <td>
                    {{ maintenance.address.addressLine }},
                    {{ maintenance.address.city }},
                    {{ maintenance.address.state }},
                    {{ maintenance.address.zipCode }}
                  </td>
                  ...
                  <td>
                    <b-button
                      variant="success"
                      v-on:click="markAsDone(maintenance.maintenanceId)"
                      >Mark as done
                    </b-button>
                  </td>
                </tr>
...
export default {
  ...
  created() {
    maintenanceService
      .retrieveMaintenancesByOwnerId(this.$store.state.user.id)
      .then((response) => {
  ...
  methods: {
    markAsDone(maintenanceId) {
      this.maintenance.status = "Done";
